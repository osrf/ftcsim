...
/////////////////////////////////////////////////
void FTC2::Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
{
  ...
  this->joystickSub = this->rosNode->subscribe<sensor_msgs::Joy>(joyTopic,
      128, &FTC2::JoystickCB, this);

  // Connect to the world update event.
  this->updateConn = event::Events::ConnectWorldUpdateBegin(
      boost::bind(&FTC2::UpdateCB, this));
}

/////////////////////////////////////////////////
void FTC2::JoystickCB(const sensor_msgs::Joy::ConstPtr &_msg)
{
  this->fwd = _msg->axes[1] * 150.0;
  this->side = _msg->axes[0] * -150.0;
  this->rotate = _msg->axes[3] * 5.0;
  this->lift = 0;
  this->claw = 0;
  this->height = 0.5;
